...
├── stl/
│   ├── bottom.stl            # 底壳 STL (3107 三角面, 单连通开口壳, 89.1 cm³)
│   ├── top.stl               # 顶盖 STL (2126 三角面, 单连通水密, 64.2 cm³, 含坐腔筒 z2.5-32)
│   └── rods.stl              # 按压杆 ×4 + 肩键滑块 ×2 (424 三角面, 3.5 cm³) - 独立小件
...
- **M3 螺丝柱 ×4**（±68,±38，z 2.5–29.2，法兰嵌入底壁）：顶盖 M3 沉孔螺钉紧固；柱内 φ2 贯穿孔（孔底穿至 z −1.5，越过支脚底，避免布尔相切残边）
...
- **坐腔筒 155.5 × 94.5 × 29.5 mm（z 2.5–32）** 嵌底壳腔（156 × 95，单边 0.25 mm 装配间隙），**筒底 2.5 坐底壳腔底承重**；筒顶 = 面板盘 155.5 × 94.5 × 2.5 mm（z 29.5–32 凹嵌底壳口内 2 mm，**面板盘与筒同宽，外缘环带已收窄，不嵌口环**），**单连通、水密**
...
- **M3 沉孔 ×4**（±68,±38）：面板螺钉孔（沉头 φ5 × 0.5 + φ2.2 通）
...
| 用量 | 底壳 89.1 cm³ / 顶盖 64.2 cm³ / 按压杆+滑块 3.5 cm³（实心体积；20% 填充实际约 底壳 22 g / 顶盖 16 g，PLA 1.24 g/cm³） |

## 关键设计验证

| 检查项 | 结果 |
|---|---|
| **STL 水密（边界边 = 0）** | ✅ top 2126 tris / rods 424 tris 边界 0；**bottom 3107 tris 边界 3**（φ2 孔口 0.31 mm 级浮点残边，切片软件自动修复，打印无影响，已记录） |
| **STL 连通（无漂浮件）** | ✅ bottom 单连通 1 件 / top 单连通 1 件 / rods 6 件（4 杆 + 2 滑块，符合设计） |
| **STL 体积断链** | ✅ bottom 89.1 cm³（开口壳，非实心 547）/ top 64.2（含坐腔筒）/ rods 3.5 —— 防"实心网格假单连通"暗病 |
...
| **装配干涉（BVH overlap，世界 = 局部 + loc 合成）** | ✅ 0 干涉（外壳 vs 全部元件）；front vs back 间隙 0.25 mm/边（筒壁/面板盘 vs 腔壁）达成 |
| **元件世界坐标** | ✅ PCB z[15.4,17]、Marmota z[4.4,15.4]、摇杆 17–27、**按压杆 z[−0.2,10.7]（loc 17.0，杆顶 10.7 距扳机底 10.9 间隙 0.2）**、**滑块 z[12.5,33.5]（帽露面板 1.5）** |
