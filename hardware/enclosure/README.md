...
│   ├── top.stl               # 顶盖 STL (2150 三角面, 单连通水密, 61.6 cm³, 含坐腔筒 z2.5-30, 滑块孔 11.6×14.6)
│   └── rods.stl              # 按压杆×4 + 肩键滑块×2(v6斜楔) + 键帽×13 + 编码器延长×1 (676 三角面, 8.1 cm³) - 独立小件
...
| 用量 | 底壳 87.5 cm³ / 顶盖 61.7 cm³ / 小件 8.1 cm³（实心体积；20% 填充实际约 底壳 22 g / 顶盖 15 g，PLA 1.24 g/cm³） |

## 关键设计验证

| 检查项 | 结果 |
|---|---|
| **STL 水密（边界边 = 0）** | ✅ **3/3 全水密**：**bottom 3044 / top 2150 / rods 676，边界边均 0 |
| **STL 连通（无漂浮件）** | ✅ bottom 单连通 1 件 / top 单连通 1 件 / rods 20 件（4 杆 + 2 滑块 + 13 键帽 + 1 编码器延长套接杆，符合设计；滑块楔块包络 10×14.4 穿面板孔 11.6×14.6） |
| **STL 体积断链** | ✅ bottom 87.5 cm³（开口壳，无冗余 msupp）/ top 61.6（含坐腔筒）/ rods 8.1（20 件含键帽延长） —— 防"实心网格假单连通"暗病 |
