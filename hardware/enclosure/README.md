...
> 风格：**按原版 Alpakka 曲面手柄造型重新设计**（161 × 100 mm 圆角矩形，角 R20、壁 2.5 mm），USB-C 从 -Y 短边出线，摇杆/编码器/按键从面板开孔穿出，**4 根独立按压杆（L2/R2/L4/R4）+ 2 根肩键楔形滑块 + 4 根 M3 螺丝柱（实心自攻咬合——几何无预钻孔，见底壳节）**。
...
│   ├── top.stl               # 顶盖 STL (2148 三角面, 单连通水密, 61.6 cm³, 含坐腔筒 z2.5-30（筒壁 USB 槽 14×8 z11-19 插头穿梭带）, 滑块孔 11.6×14.8（沉槽 14.2×12.0 收窄帽晃 1.0→0.2）)
...
| PCB 支撑 | 支撑块 ×4（12×12×12.8 @±60,±30），与薄壳一体（挖腔留柱）；主板 xy 定位由元件穿孔自对中（装配步 2——原设计定位小台未建模，见 L90 注） |
...
- **支撑块 ×4**（±60,±30，z 2.5–15.3）：主板（顶面 z17）落在支撑块顶（15.3）承重；**主板 xy 由 13 按键 + 摇杆杆 + 编码器杆穿面板孔自对中（装配步 2）——原设计定位小台 ×4（10×10×1.8 @±28,±28）经 blend/STL 复核未建模，声明移除**
...
- **USB-C 槽**：14 × 30 mm 贯通 -Y 短边壁（外口 y −50 → 内口 y −48，穿壁 2 mm；槽体 z 0..29.7 全高贯通——blend 实测（旧文 8 mm 高为误——槽容插口 z7-13 与插头穿梭，30 高必需）；槽体向下延伸至 z −0.3 越过底壁外表面，避免布尔相切），直达 Marmota J1 插口端（主板座中心 y 34.19 → Blender −26.31；连接器端面偏 +0.46 → 插口端 −25.85，blend 实测）；**筒壁对应槽 14×8（z 11–19——插头穿梭带，blend 实测——非全高，顶壁 0–11/19–30 保持强度；插头斜下穿带入插口）**
...
| J1 | USB-C（Marmota 下边缺口内，rot 180°） | (0, −21.6)，插口端 −25.85 | 6 | 底壳 -Y 壁 14×30 贯通槽（z0–29.7 全高——旧文 14×8 为误，同 L94/L160） |
...
| 打印方向 | 顶盖**面板朝下**（面板盘贴床、筒口朝上，打印顺序：面板盘顶 30 → 面板盘 2.5 mm 厚 → 筒壁环向下 27.5 mm（z27.5→2.5）至筒口 2.5，全程竖直壁无悬垂）；底壳**开口朝上**；按压杆**立放**（杆体垂直）；滑楔滑块**帽朝下**（帽 30.5–32 平底贴打印面，帽面平整；楔块底 8 为最高层，孔口朝上无悬垂；斜面 17° 近竖直无需支撑） |
| 水平扩展（Horizontal Expansion） | −0.05 mm（孔位更精确） |
| 用量 | **材料体积估算（壁展开法交叉验证）：底壳 81.5 cm³（底壁 37.0+侧壁 39.5+细节 5）/ 顶盖 74.1 cm³（面板盘 36.7+筒壁 34.4+细节 3）/ 小件 9.3 cm³（实心）→ PLA 1.24 g/cm³：底壳 ~101 g / 顶盖 ~92 g / 小件 ~11.5 g / 合计 ~205 g**（壁 2.5 实心为主体；20% 填充仅影响壁内空腔，对薄壁壳影响 <5%；⚠️ 原『87.5/61.6/9.3 cm³ 实心体积 + 40 g』为包围体积 × 填充系数双重误算、低估约 5 倍——已修正；**精确用量以切片器估算为准**） |
...
| 整机重量估算 | 外壳 ~205 g（PLA）+ 电子件 ~60 g（主板 15+Marmota 10+按键/摇杆/编码器 25+帽盘/旋钮 10）= **整机 ~265 g**——手持适中（PS 手柄 ~250 g 同级）；PETG 外壳 +2.4%（密度 1.27 vs PLA 1.24：205×1.27/1.24≈210 g）→ 整机 ~270 g |
...
>
> **底壳打印注意（USB 槽顶桥接）**：−Y 壁 USB 槽（14×30，z 0..29.7 全高贯通——blend 实测；旧文 14×8 为误）贯穿壁厚 2.5；槽顶（z29.7）为壁环恢复实心的第一层——桥接面 2.5（短边）×14（长边）。切片器默认桥接沿短边（2.5，轻松）；若切片器沿 x 长边（14）则接近桥接极限，可在切片设置中强制桥接方向或加 0.5 mm 槽顶圆角。
...
> **整机成本估算（参考）**：打印材料 ~35 元（PLA 250g 卷）+ 电子件 ~275 元（主板 150 / Marmota 80 / 按键·摇杆·编码器·肩键·扳机 40 / M3 螺丝 5 及备件——分项和 275）+ 外购帽盘·旋钮 ~20 元 ≈ **整机 ~330 元**；PETG 材料 +20 元。不含运费与工具（卡尺/螺丝刀）。

## 关键设计验证

> 验证表分类：**套件 `enclosure_verify.py` v12 自动回归**（水密/自相交哨兵/切片/拓扑/连通/体积/装配间隙/运动学/法线等 12 项）；**其余为 Blender BVH/SAT/手工/临时脚本实测**（已逐行标注的 L183 孔间壁厚、L191 装配干涉、L214 滑块间距等——行号随文档编辑漂移，以 BVH 标注字样检索为准）——任何几何修改后重跑套件并按标注复查手动项。

| 检查项 | 结果 |
|---|---|
| **STL 水密（边界边 = 0）** | ✅ **3/3 全水密**：**bottom 3044 / top 2148 / rods 804（v6e），边界边均 0 |
...
| **面板布局两两间距（18 凸出件 2D 包络——帽盘 2 + 旋钮 1 + 键帽 13 + 滑块帽 2；摇杆杆在孔内不凸出不计）** | ✅ 6 对 <3 mm：最紧 **S4/S5 键帽 vs 滑块帽 0.70**（S4=Keycap_1 (−49,−15.5) 对 L1 帽区左缘 x 距 0.70；S5 (49,5.5) 对 L2 帽区右缘 0.70——对称两对，blend 实测帽区包络）（静态间隙无运动碰撞，公差余 0.3）、旋钮-键帽 0.90（约束 Ø16 已定）、S2/S7 1.40、U2 帽盘占位-S5 键帽 0.01（Ø26 渲染占位相触不打印——真实帽盘 Ø15 vs S5 键帽 6.99 安全）；其余 2 对 2.0–3.0 之间（非最紧，均安全未逐列） |
...
| **STL 体积断链** | ✅ bottom 87.5 cm³（开口壳，无冗余 msupp）/ top 61.6（含坐腔筒）/ rods 9.2（20 件含键帽延长） —— 防"实心网格假单连通"暗病 |
| **按压极限动态（凸出件按下极限 z）** | ✅ 帽盘轴向按下 1 → 帽盘顶 33 > 面板顶 30（不碰——静止顶 34 按下 1）；键帽按下 1.5 → 键帽顶 29 < 30（齐面板）；滑块帽按下 1.5 → 帽底 29 入沉槽（导向 0.2/0.3 限位）；按压杆触发 0.45 → 杆帽底 −0.65 > 支脚 −1（余 0.35 不触地）、超行程 0.7 → −0.9（余 0.1）；旋钮旋转无位移 |
| **面板孔间壁厚扫描（18 孔两两）** | ✅ 最薄 1.10 mm（U2 摇杆孔 21×21 @34,14.5 vs S5 键帽孔 6.8 @49,5.5——短桥 2.5 厚可打印 2–3 圈、可承摇杆按压力）；其次 S15-L1 滑块 1.30（S15 肩键孔对应 Slider_L1）、S4/S8-滑块 1.36；其余 ≥2.00；全部 ≥1.0 打印与强度安全（BVH 实测——孔位修改后须复查） |
...
| **文档改动后几何回归（最终）** | ✅ 三 STL 水密 0/0/0、体积 87.5/61.6/9.2、SHA-256 与校验和表吻合——多轮 README 修改未影响几何文件（git 仅改文档）；壁 2.5=6.25 圈（0.4 喷嘴，切片器按壁厚自动）、PLA 同缩间隙保持、水平扩展 −0.05 使孔 0.2→0.15（QA 项 4 覆盖） |
...
| **一体式结构（暗病修复：UNION 融合陷阱）** | ✅ 支撑/螺丝柱以"挖腔留柱"实现（cutter 带洞差集），与薄壳同源材料、必然单连通；原"结构件 UNION 进薄壳"方案在 obj 级布尔下多次出现 2 分量（结构件漂浮），已弃用 |
...
**装填件（外购/PCB）**：主板 + Marmota 模块、摇杆 RKJXV ×2、编码器 EC10E、按键 SKPM 6×6 ×13、肩键 SKHHL ×2、扳机 SKQG ×4、M3×12 沉头螺丝 ×4。

**仅 Blender 装配/渲染占位（不打印）**：`PCB_Reference` 集合（PCB 板形 + 全部元件占位）、**摇杆帽盘（Ø26 × 4 mm 高 ×2 个——U1/U2 各一，blend 对象 JoyProbe_U1/U2_cap）** —— 摇杆帽为造型占位（帽盘不套杆仅为自由放置座面示意）；**编码器旋钮未建渲染占位**（外购件——见 BOM 采购清单——渲染图中编码器区无旋钮造型），打印件均不含此件。
...
| M3 通孔 Ø3.3 vs 螺丝 Ø3.0 | 0.15 | 过孔 | 水平扩展 −0.05 修正后 0.05–0.35——极限过盈时自攻丝硬压带出（QA 7 实测——M3×12 装配项） |
...
> 编号 9（推杆伸出）已并入 4b——推杆检查唯一化。QA 项 0a：按键到货实测高度，±0.5 时键帽底 21.5 需随动（改键帽长或按键垫高）；0b：扳机实测高度，杆-扳机 0.2 间隙链以装配高 4.5 为基准（体 4.3 + 焊脚 0.2——扳机底 10.9 = PCB 底 15.4 − 4.5），3.4 空程 1.3（4.5−3.4+0.2）/ 5.0 预压 0.3（5.0−4.5−0.2）均需调整杆长。

## BOM 采购清单（装填件，对照元件表）

> **备件建议**：M3×12×4 多备 4–8 颗（自攻 PLA 滑丝后可换——滑丝处置见 QA 7）；滑丝孔补救：实心柱裂/滑丝→**自钻 φ2.5 底孔（自攻路径）或扩孔 φ4.5 热熔压入 M3 铜螺母（M3×4.5——柱 φ6 壁余 0.75）或换位锁紧**（φ2.5 孔容不下铜螺母 OD4.5——两路径二选一）。键帽/滑块/按压杆/延长为可打印件——掉件/断角直接重打（备料双卷已含重打余量）。
...
| 主板 | 1 | 130×71，USB-C 朝 −y | 支撑块×4（xy 穿孔自对中） |
| Marmota | 1 | 70×70，板对板 DF12×2 | 主板下方 3 mm 间隙 |
| 摇杆帽盘（外购） | 2 | ≤Ø15（适配 21×21 孔，摆 10.9° 余 0.68；渲染占位 Ø26 不打印） | 摇杆孔 21×21，套高帽杆 Ø10 |
| 编码器旋钮（外购） | 1 | ≤Ø16（约束：旋钮-键帽 0.90 间隙） | 延长套杆 Ø6.2×6（z26–32） |

> **rods.stl 对应装填件**（杆 4=扳机 4、滑块 2=肩键 2、键帽 13=按键 13、延长 1=编码器 1）一一对应，无冗余采购；**外购帽盘 ×2 / 旋钮 ×1 为独立装填件**（非 rods 打印件——含于上表，采购勿漏）。
...
2. **放主板**：主板（130×71，底 15.4）落入底壳，底面坐 4 个支撑块（±60,±30，顶 15.3）承重；按压杆顶 10.7 与扳机底 10.9 留 0.2 间隙不预压；**方向**：USB-C 插口朝 −y（背壳槽侧）——主板 180° 装反能放平（支撑块对称）但 front 面板孔对不上元件、压不到底（天然防错）；**插头行程**：槽内口（y −48）到 J1 插口端（y −25.85，blend 实测 J1_USB_C -y 端）22.15 mm，USB-C 插头+线缆柔直伸可及（无需对孔导向）；**xy 平面定位**：支撑块仅承 z 向，主板 xy 由 13 按键 + 摇杆杆 + 编码器杆穿面板孔自对中（最小约束 0.4/边，晃动 ≤0.4 mm 不影响触发）
...
| 编码器延长 | 1 | 0.096 cm³（含盲孔 Ø6×3 掏空——实心柱 0.181−孔 0.085） | 0.096 | 竖放（内孔朝床，孔口向下凹 3 mm） |
| **合计** | **20** | — | **9.18 cm³**（1.76+3.16+4.16+0.096——旧 9.26 含延长实心 0.18 未扣盲孔——已修） | rods.stl 单文件导入切片器后按件分离摆放 |
...
| 6 | 摇杆帽盘 | ≤Ø15、摆动 10.9° 不碰孔缘/相邻件 | 换小帽盘/扩孔 22×22（Ø17 上限——26×26 与 S8 孔重叠 1.4 穿孔不可行，须先重排 S8） |
| 7 | M3×12 ×4 | 对角锁紧、咬合 9.5 mm、无滑丝 | 实心柱直拧裂/滑丝→自钻 φ2.5 底孔 / 扩孔 φ4.5 压 M3 铜螺母 / 换螺丝（双路径二选一） |
...
| 5 | 摇杆帽盘直径（外购） | 采购时 | ≤ Ø15（Ø16 临界 0.19 余量） | 换小帽盘/扩孔 22×22（Ø17 上限——26×26 与 S8 孔重叠 1.4 穿孔不可行，须先重排 S8） |
...
| 螺丝锁不紧 / 滑牙 | 自攻咬合不足 / 实心直拧裂 | 自钻 φ2.5 底孔 / 扩孔 φ4.5 压 M3 铜螺母 |
...
| 螺丝柱孔滑牙 | 自攻过紧 / 层间弱 | 自钻 φ2.5 底孔 / 扩孔 φ4.5 压 M3 铜螺母（见 BOM 备件） |
...
维护：硬件外壳与 PCB 同步更新。**QA 双表同步**：`QA_first_build.md` 模板由本 README 的 QA 清单复制而来——任何 QA 项（编号/标准/处置）修改后必须同步更新模板（含 SHA 表行），防止双表漂移。任何 PCB 修改后应重新核对 STL 关键尺寸（板框、摇杆/编码器/按键位置、USB-C 位置、Marmota 叠层高度、元件旋转角），并重跑验证（水密 / 连通 / 体积断链 / 干涉）；手动项（BVH 孔间壁厚 / 装配干涉 / 滑块间距——见验证表分类 L170 标注行）按标注逐一复查。
