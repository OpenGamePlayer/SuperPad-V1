...
├── stl/
│   ├── bottom.stl            # 底壳 STL (4141 三角面, 单连通, 126.0 cm³)
│   ├── top.stl               # 顶盖 STL (2148 三角面, 单连通水密, 57.3 cm³)
...
- 开口朝上，内腔 156 × 95 × 29 mm（z 2.5–31.5），薄壳 + 一体结构，**单连通**
...
- 外形筒 **155.5 × 94.5 × 21 mm（z 11–32）** 嵌底壳内腔（156 × 95，单边 0.25 mm 装配间隙）+ 面板层 2.5 mm（161 × 100，z 29.5–32 坐底壳口缘），**单连通、水密**
...
| 打印方向 | 顶盖**面板朝上**；底壳**开口朝上**；按压杆**立放**（杆体垂直）；滑楔滑块**帽朝上** |
| 水平扩展（Horizontal Expansion） | −0.05 mm（孔位更精确） |
| 用量 | 底壳 126.0 cm³ / 顶盖 57.3 cm³ / 按压杆+滑块 3.5 cm³（实心体积；20% 填充实际约 底壳 31 g / 顶盖 14 g，PLA 1.24 g/cm³） |

## 关键设计验证

| 检查项 | 结果 |
|---|---|
| **STL 水密（边界边 = 0）** | ✅ top 2148 tris / rods 424 tris 边界 0；**bottom 4141 tris 边界 3**（φ2 孔口 0.31 mm 级浮点残边，切片软件自动修复，打印无影响，已记录） |
| **STL 连通（无漂浮件）** | ✅ bottom 单连通 1 件 / top 单连通 1 件 / rods 6 件（4 杆 + 2 滑块，符合设计） |
| **STL 体积断链** | ✅ bottom 126.0 cm³（薄壳 ~117.6 + 结构 ~8.4，非实心 547）/ top 57.3 / rods 3.5 —— 防"实心网格假单连通"暗病 |
| **装配干涉（BVH overlap，世界 = 局部 + loc 合成）** | ✅ 0 干涉（外壳 vs 全部元件）；滑块 vs 外壳 overlap 为浮点误报（back 腔解剖无材料、最近点距离 0.5 mm 无接触） |
