...
> 验证表分类：**套件 `enclosure_verify.py` v12 自动回归**（水密/自相交哨兵/切片/拓扑/连通/体积/装配间隙/运动学/法线等 12 项）；**其余为 Blender BVH/SAT/手工/临时脚本实测**（已逐行标注的 L180 孔间壁厚、L188 装配干涉、L210 滑块间距等）——任何几何修改后重跑套件并按标注复查手动项。
