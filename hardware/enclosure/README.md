...
- [目录结构](#目录结构)
- [几何参数](#几何参数)
- [元件位置（Blender 单位 mm，外壳坐标系：底壳底 z=0，PCB 顶面 z=17）](#元件位置blender单位mm外壳坐标系底壳底z=0pcb顶面z=17)
- [打印参数建议](#打印参数建议)
- [关键设计验证](#关键设计验证)
- [打印件清单 vs 占位件（防混淆）](#打印件清单vs占位件防混淆)
- [关键配合间隙表（打印前参考，单位 mm）](#关键配合间隙表打印前参考单位mm)
- [占位件尺寸假设清单（可靠性分级）](#占位件尺寸假设清单可靠性分级)
- [BOM 采购清单（装填件，对照元件表）](#bom采购清单装填件对照元件表)
- [外购件尺寸约束（防碰撞）](#外购件尺寸约束防碰撞)
- [装配说明书（8 步）](#装配说明书8步)
- [rods.stl 件数体积分布（打印排布参考）](#rodsstl件数体积分布打印排布参考)
- [交付文件校验和（SHA-256，下载后验证完整性）](#交付文件校验和sha256下载后验证完整性)
- [嘉立创 EDA 协作工作流](#嘉立创eda协作工作流)
- [打印首版实测 QA 清单（装机后逐项验收）](#打印首版实测qa清单装机后逐项验收)
...
| 维度 | 数值 |
|---|---|
| 外尺寸（X×Y×Z） | 161.0 × 100.0 × 34.0 mm（底壳体高 34，含支脚底 −1.0 总高 35）；**含凸出件包络**：旋钮顶 36 为最高点（凸 2）、帽盘/滑块帽/键帽 30.5–34 齐口环内——整机包络 161 × 100 × 36（携行/收纳参考） |
...
- **肩键滑块 v6d ×2（S15_L1 / S16_R1，世界坐标构建，loc 0）**：**帽 13.6 × 11.6 × 1.5 mm（z 30.5–32，露面板顶 1.5）+ 颈 10 × 9 × 13.5 mm（z 17–30.5）+ 楔块 10 × 14.4 mm（z 8–24，中央孔 8.0 × 7.4 套 7.5 × 6.8 SKHHL 体，-y 壁下部斜面 z8–16 斜率 0.31 推 SKHHL 水平推杆端）**—— 滑块下压 0.81 mm（帽行程 1.0 内）→ 推杆 +Y 压回 0.25 触发，松手由 SKHHL 内部弹簧回弹
...
| 用量 | 底壳 87.5 cm³ / 顶盖 61.6 cm³ / 小件 9.3 cm³（实心体积；20% 填充实际约 底壳 22 g / 顶盖 15 g / 小件 3 g / 合计 ~40 g，PLA 1.24 g/cm³） |
...
| **STL 自相交精确测试（Moller SAT）** | ✅ 3/3 无真实穿透：候选 226/35/76 对经 SAT 分离轴精测后全部为**共面相邻细分面**（STL 独立三角格式无顶点索引所致，位置集中于外壳外壁 x=±80.5、面板筒壁 x=±77.8、圆柱底圆扇形）——非几何交叉；共面重合 5 万对为布尔平面细分正常特征，切片软件自动合并修复 |
| **交付对象-文件对应（58 网格对象审计）** | ✅ 打印件 22（NB_back + NB_front + rods 20）全部导出、占位 36（PCB/Marmota 2 + 元件 30 + JoyProbe 4）不导出、无冗余 rods 对象；渲染/打印边界零混淆 |
| **面板布局两两间距（18 凸出件 2D 包络）** | ✅ 6 对 <3 mm：最紧 **S1/S5 键帽 vs 滑块帽 0.70**（静态间隙无运动碰撞，公差余 0.3）、旋钮-键帽 0.90（约束 Ø16 已定）、S2/S7 1.40、U2 帽盘-S8 2.00；全部安全 |
...
| **面板孔间壁厚扫描（18 孔两两）** | ✅ 最薄 1.10 mm（U2 摇杆孔 21×21 @34,14.5 vs S8 键帽孔 6.8 @49,5.5——短桥 2.5 厚可打印 2–3 圈、可承摇杆按压力）；其次 S0-L1 滑块 1.30、S4/S8-滑块 1.36；其余 ≥2.00；全部 ≥1.0 打印与强度安全 |
| **运动学验证（3D 采样）** | ✅ 帽盘 Ø15 绕球窝 z17 摆 10.9°（48 方位×24 边缘采样）：穿出孔缘 0.000 mm、边缘最低 z 28.35（>孔底 27.5 不穿面板盘）；Ø20 会穿出 1.78（已禁）；滑块链推杆 0.25→楔块斜 0.31→下移 0.81<帽行程 1.0（余 0.19）；旋钮旋转无位移（与 S14 键帽 3D 距 1.03）；平动件（键帽 1.5/滑块 1.5/按压杆 0.7）按下时 z 分离更安全 |
...
| **STL 法线字段（格式审计）** | ✅ 三 STL 法线字段全零（Blender 导出标准——不写单位法线字段）；顶点顺序（切片器实际依据）多数向外正确——Cura/PrusaSlicer 忽略法线字段、OpenSCAD 等读零法线工具自动重算，均不影响打印；⚠️ 此前『100% 反向』为诊断脚本误报（零向量 cos=0 被误判反向），已更正 |
...
**仅 Blender 装配/渲染占位（不打印）**：`PCB_Reference` 集合（PCB 板形 + 全部元件占位）、**摇杆帽盘（Ø26 × 4）**、旋钮 —— 摇杆帽/旋钮为造型占位，实际使用外购摇杆帽与编码器旋钮；帽盘不套杆仅为自由放置座面示意，打印件不含此件。

## 关键配合间隙表（打印前参考，单位 mm）

| 配合对 | 间隙（每边） | 类型 |
|---|---|---|
| front 筒 vs back 腔壁 | 0.25 | 滑配（筒插入腔） |
| 面板盘外缘 vs 口环 | 0.25 | 滑配（凹嵌 4 mm） |
| 滑块楔块 10×14.4 vs 滑块孔 11.6×14.8 | 0.8 / 0.2 | 穿入（0.2 紧，打印实测） |
| 滑块帽 13.6×11.6 vs 沉槽 14.2×12.0 | 0.3 / 0.2 | 行程导向（帽按下不晃） |
| 键帽 6×6 vs 面板孔 6.8×6.8 | 0.4 | 穿入+导向 |
| 摇杆杆 Ø10 vs 摇杆孔 21×21 | 5.5 | 摆动余量（帽盘 ≤Ø15 摆 10.9° 边缘余量 ≥0.67；Ø20 会穿出孔缘 1.78 不可用） |
| 编码器延长 Ø6.2 vs 面板孔 Ø13 | 3.4 | 穿出 |
| 按压杆帽 8.5 vs 扳机孔 9.0 | 0.25 | 滑动导向 |
| 楔块孔 8.0×7.4 vs SKHHL 体 7.5×6.8 | 0.25 / 0.3 | 套入间隙 |
| 滑块颈 10×9 vs 滑块孔 | 0.8 / 2.9 | 大间隙（定位靠帽沉槽） |
| M3 通孔 Ø3.3 vs 螺丝 Ø3.0 | 0.15 | 过孔 |
| 按压杆顶 10.7 vs 扳机底 10.9 | 0.2 | 防预压间隙 |
...
| 编码器 EC10E | 杆 Ø6 × 9 | 标准规格 | ⚠️ **杆高变体 8–10** | 延长套杆：杆高 9 套 3 mm、10 套 2 mm（够）；**<8.5 时延长底 26 悬空不套杆、旋钮不转** | QA 0c 实测杆高 |
...
> QA 项 0a：按键到货实测高度，±0.5 时键帽底 21.5 需随动（改键帽长或按键垫高）；0b：扳机实测高度，杆-扳机 0.2 间隙链以 4.3 为基准，3.4 空程 1.3 / 5.0 预压 0.3 均需调整杆长。
...
| M3×12 | 4 | 沉头/盘头（头 Ø5.5 入沉孔 Ø6） | 柱孔 Ø2.5 自攻咬合 9.5 |
...
| 外购件 | 约束 | 计算依据 |
|---|---|---|
| 摇杆帽盘 | **≤ Ø15**（3D 运动学：帽盘边缘绕球窝 z17 摆 10.9° 位移 0.982R+2.458 ≤ 孔缘 10.5 → 帽半 R ≤ 8.19；Ø15 余量 0.67、Ø16 临界 0.19、Ø20 穿出 1.78 不可用） | 帽半 ≤ 8.19 |
...
2. **放主板**：主板（130×71，底 15.4）落入底壳，底面坐 4 个支撑块（±60,±30，顶 15.3）、定位小台（±28,±28）限位；按压杆顶 10.7 与扳机底 10.9 留 0.2 间隙不预压；**方向**：USB-C 插口朝 −y（背壳槽侧）——主板 180° 装反能放平（支撑块对称）但 front 面板孔对不上元件、压不到底（天然防错）；**插头行程**：槽口（−y 壁）到 J1 插口 21.7 mm，USB-C 插头+线缆柔直伸可及（无需对孔导向）；**xy 平面定位**：支撑块/小台仅承 z 向，主板 xy 由 13 按键 + 摇杆杆 + 编码器杆穿面板孔自对中（最小约束 0.4/边，晃动 ≤0.4 mm 不影响触发）
3. **套顶盖**：front 筒（155.5×94.5）插入底壳腔（156×95，0.25/边），筒内腔自动套过全部元件；筒底坐腔底、面板盘（27.5–30）凹嵌口环 4 mm、柱顶 27.5 与面板盘底共面承力
...
| 2 | 按压杆 ×4 | 帽穿底孔滑动无卡滞、杆顶距扳机 0.2 | 孔口倒角 |
| 3 | 键帽 ×13 | 穿面板孔顺滑、按下按键触发回弹 | 孔 6.8 微扩 |
| 4 | 肩键滑块 ×2 | 楔块穿面板孔套 SKHHL、下压 0.81 触发 | 孔 14.8 微扩 |
...
| 3b | 编码器杆高（QA 0c） | 到货实测 | 9（≥8.5） | 延长内孔加深/换长杆型号 |
...
2. **跑验证套件**：`python verify/enclosure_verify.py` → 9 项全 PASS 再开工
...
- [ ] 大帽盘升级路径：若需 Ø18+ 帽盘（握持感），须同步将摇杆孔 21×21 扩至 26×26（孔缘半 13 ≥ 9·0.982+2.458=11.30 摆后），并复查与相邻件间距
