...
> **填写示例**（演示格式——不是真实数据）：`| 0a | 按键 SKPM 高度 | 4.5 ± 0.5… | 键帽加长 0.5… | 4.48 | ✓ | 卡尺读数 4.48 |`；失败则 `| 2 | 按压杆 ×4 | … | 4.2（<4.5）| ✗ | 杆长微调 ±0.2 后复测 |`。
...
| 0 | **Marmota 高度实测** | DF12 板对板（2×20P）间隙 3 mm 计入后模块底 ≥ 腔底 2.5（当前假设顶贴 PCB 15.4、底 4.4；若实际模块顶 12.4 则底 1.4 穿入腔底 1.1） | 抬高主板位（占位同步）/换矮 DF12/减模块高度 | | | |
